...
This can be done using multilevel correlation and is analogous to use a linear mixed model (LMM) with the grouping variable treated as a random effect.
Let's use the function `correlation` from the `correlation` package to get the Pearson correlation with and without this multilevel structure in which the sample are nested inside the groups:
